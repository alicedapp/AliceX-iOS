...
+ (void)updateFloatImage:(UIImage *)image {
    [HKFloatManager shared].floatBall.iconImageView.image = image;
}
...
- (void)floatBallDidClick:(HKFloatBall *)floatBall {
    if ([NSStringFromClass([self.floatViewController class]) isEqualToString:
         NSStringFromClass([[self hk_currentViewController] class])]) {
        return;
    }
...
- (BOOL)haveIconImage {
    ...
    for (unsigned int i = 0; i < outCount; i++) {
        ...
        if ([nameStr isEqualToString:@"hk_iconImage"]) {
            have = YES;
        }
    }
    free(ivars);
