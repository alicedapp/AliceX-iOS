...
open class LBXScanWrapper: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    open func captureOutput(_: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from _: AVCaptureConnection!) {
        if !isNeedScanResult {
            // 上一帧处理中
            return
        }
    ...
    public static func createCode128(codeString: String, size _: CGSize, qrColor _: UIColor, bkColor _: UIColor) -> UIImage? {
        let stringData = codeString.data(using: String.Encoding.utf8)
